...
$defaultRates = [
    ...
    'RWF'=>1350,'ZMW'=>27,'SLE'=>22,'TZS'=>1,'GHS'=>1,'NGN'=>1,
];
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['mobile_submit'])) {
    ...
    $minAmount = ($currency === 'KES') ? 4 : (int)($defaultRates[$currency] ?? 1);

    if (!$dialCode || !$currency) {
        $error = "Pays non reconnu. Utilisez +254 (Kenya/KES), +256 (Uganda/UGX) ou +243 (Congo/CDF).";
...
<div class="hist-wrap">
  ...
  <div class="dep-item">
    ...
    <div class="dep-right">
      <div class="dep-amt <?= $st ?>">$<?= number_format((float)$dep['amount'], 4) ?></div>
...
const currencyMap = {'243':'CDF','254':'KES','256':'UGX'};
