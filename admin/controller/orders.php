...
if( route(2)  ==  "provider" ):
  ...
  if( $_SESSION["client"]["data"] ):
    ...
    if( $_GET["search_type"] == "username" && $_GET["search"] ):
      ...
      elseif( $_GET["search_type"] == "multi_id" && $_GET["search"] ):
      $search_where = $_GET["search_type"];
      $search_word  = urldecode($_GET["search"]);
      $ids = preg_split('/[\s,]+/', $search_word);
      ...
      if (!empty($ids)) {
          ...
          $count = $conn->prepare("
              ...
              WHERE (
                  orders.order_id IN ($in) 
                  OR orders.service_id IN ($in) 
                  OR orders.api_orderid IN ($in) 
                  OR orders.api_serviceid IN ($in)
              )
              && orders.dripfeed='1' && orders.subscriptions_type='1'
          ");
          $count->execute();
          $count = $count->rowCount();

          $search = "WHERE (orders.order_id IN ($in) 
                        OR orders.service_id IN ($in) 
                        OR orders.api_orderid IN ($in) 
                        OR orders.api_serviceid IN ($in))
                        && orders.dripfeed='1' && orders.subscriptions_type='1' ";
    ...
    elseif( $_GET["search_type"] == "api_serviceid" && $_GET["search"] ):
      $search_where = $_GET["search_type"];
      $search_word  = urldecode($_GET["search"]);
      $count        = $conn->prepare("SELECT * FROM orders INNER JOIN clients ON clients.client_id = orders.client_id WHERE orders.api_serviceid LIKE '%".$search_word."%' && orders.dripfeed='1' && orders.subscriptions_type='1' ");
      $count        -> execute(array());
      $count        = $count->rowCount();
      $search       = "WHERE orders.api_serviceid LIKE '%".$search_word."%'  && orders.dripfeed='1' && orders.subscriptions_type='1' ";
      $search_link  = "?search=".$search_word."&search_type=".$search_where;  
    elseif( $_GET["search_type"] == "api_orderid" && $_GET["search"] ):
      $search_where = $_GET["search_type"];
      $search_word  = urldecode($_GET["search"]);
      $count        = $conn->prepare("SELECT * FROM orders INNER JOIN clients ON clients.client_id = orders.client_id WHERE orders.api_orderid LIKE '%".$search_word."%' && orders.dripfeed='1' && orders.subscriptions_type='1' ");
      $count        -> execute(array());
      $count        = $count->rowCount();
      $search       = "WHERE orders.api_orderid LIKE '%".$search_word."%'  && orders.dripfeed='1' && orders.subscriptions_type='1' ";
      $search_link  = "?search=".$search_word."&search_type=".$search_where; 
    ...
    $orders = $conn->prepare("SELECT orders.*, clients.*, services.*, orders.completion_time FROM orders INNER JOIN clients ON clients.client_id=orders.client_id INNER JOIN services ON services.service_id=orders.service_id $search ORDER BY orders.order_id DESC LIMIT $where,$to ");

$orders->execute(array());
$orders = $orders->fetchAll(PDO::FETCH_ASSOC);
$failCount = $conn->prepare("SELECT * FROM orders WHERE order_error!=:error "); // Removed dripfeed/sub    $failCount     -> execute(array("error"=>"-"));
...
header("Location:".site_url("admin/orders"));
 ...
 elseif( route(2) == "order_complete" ):
    ...
    $update = $conn->prepare("UPDATE orders SET order_status=:status, order_error=:error, last_check=:check, completion_time=:completion_time WHERE order_id=:id");
...
function hoursTaken($startDatetime, $completionDatetime) {
    $start = new DateTime($startDatetime);
    $completion = new DateTime($completionDatetime);
    $diff = $start->diff($completion);

    // Prepare the output string
    $timeParts = [];

    if ($diff->y > 0) {
        $timeParts[] = $diff->y . ' year' . ($diff->y > 1 ? 's' : '');
    }
    if ($diff->m > 0) {
        $timeParts[] = $diff->m . ' month' . ($diff->m > 1 ? 's' : '');
    }
    if ($diff->d > 0) {
        $timeParts[] = $diff->d . ' day' . ($diff->d > 1 ? 's' : '');
    }
    if ($diff->h > 0) {
        $timeParts[] = $diff->h . ' hour' . ($diff->h > 1 ? 's' : '');
    }
    if ($diff->i > 0) {
        $timeParts[] = $diff->i . ' minute' . ($diff->i > 1 ? 's' : '');
    }
    if ($diff->s > 0) {
        $timeParts[] = $diff->s . ' second' . ($diff->s > 1 ? 's' : '');
    }
