...
if (route(4) == "delete"):
    ...
    if ($_POST):
        ...
        $adminId = htmlspecialchars($admin["admin_id"]);
        ...
        if (strlen($message) < 3):
            ...
            $update = $conn->prepare("UPDATE tickets SET canmessage=:canmessage, status=:status, lastupdate_time=:time, support_new=:new, admin=:admin, admin_name=:admin_name  WHERE ticket_id=:t_id ");
            $update = $update->execute(array("t_id" => route(3), "time" => date("Y-m-d H:i:s"), "status" => "answered", "canmessage" => 2, "new" => 2, "admin" => $adminId, "admin_name" => $adminName));
             $tr_arr =array(
                "t_id" => route(3), 
           ...
           // print_r($tr_arr); die;
                   $insert = $conn->prepare("INSERT INTO ticket_reply SET ticket_id=:t_id, time=:time, support=:support, message=:message, client_id=:client_id, admin=:admin, admin_name=:admin_name");
