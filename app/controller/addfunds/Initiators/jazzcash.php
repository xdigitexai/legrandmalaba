...
$JazzcashTransactionId = htmlspecialchars(trim($_POST["JazzcashTransactionId"]));

if (empty($JazzcashTransactionId)) {
...
if (
    !countRow([
        'table' => 'payments',
        'where' => [
            'payment_extra' => $JazzcashTransactionId,
            "payment_status" => 3,
            "payment_delivery" => 2
        ]
    ])
    ...
    $emails = imap_search($inbox, 'TEXT "' . $JazzcashTransactionId . '"', SE_FREE, "UTF-8");
    ...
    foreach ($emails as $email_id) {
        ...
        $transaction["tid"] = $JazzcashTransactionId;
    }
    imap_close($inbox);
    
    if($transaction["tid"] != $JazzcashTransactionId){
