...
$title = ($title ?? '') . ($languageArray["neworder.title"] ?? '');
...
$multiName = is_array($multiName) ? $multiName : [];
$_lang = $user["lang"] ?? '';
if( $_lang && ($multiName[$_lang] ?? null) ):
$s["service_name"] = $multiName[$_lang];
else:
...
$categories1 = $conn->prepare("SELECT * FROM categories WHERE category_type='2' AND category_deleted='0'  ORDER BY categories.category_line ASC ");
$categories1->execute(array());
...
for ($i = 0; $i < count($categories1); $i++) {
  ...
  if(is_array($multiCatName) && ($multiCatName[$user["lang"] ?? ''] ?? null)){
      $name = $multiCatName[$user["lang"]];
  }
  if(($_SESSION["selected_category_id"] ?? null) == $id && $i != 0){
...
// New: Universal duplicate link check for ALL SERVICES
$duplicateLinkCheck = $conn->prepare("SELECT COUNT(*) FROM orders 
    WHERE client_id = :client_id 
    AND LOWER(TRIM(order_url)) = LOWER(TRIM(:link))
    AND order_status IN ('pending', 'processing', 'inprogress')");
$duplicateLinkCheck->execute([
    'client_id' => $user["client_id"],
    'link' => $link
]);
$duplicateCount = $duplicateLinkCheck->fetchColumn();
  ...
  elseif ($service_detail["service_package"] == 1 && (empty($link) || empty($quantity))):
    ...
    elseif ($duplicateCount > 0): // New universal link check
    $error = 1;
    $errorText = "You have an active order with this link. Please wait until current orders complete.";
