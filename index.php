...
$gets = isset($first_route[1]) ? explode('&', $first_route[1]) : [];
foreach ($gets as $get) {
  $get = explode('=', $get);
  $_GET[$get[0]] = $get[1];
}
...
if (($_GET["lang"] ?? null) && $user['auth'] != 1):
  ...
  if (($_SESSION["lang"] ?? null) && $user['auth'] != 1):
...
if (!isset($route[0]) && ($_SESSION["msmbilisim_userlogin"] ?? false) == true) {
  $route[0] = "neworder";
  $routeType = 0;
} elseif (!isset($route[0]) && ($_SESSION["msmbilisim_userlogin"] ?? false) == false) {
  $route[0] = "auth";
  $routeType = 1;
} elseif ($route[0] == "auth" && ($_SESSION["msmbilisim_userlogin"] ?? false) == false) {
  $routeType = 1;
} else {
  $routeType = 0;
}
...
if (route(0) != "admin" && route(0) != "ajax_data") {
  ...
  foreach ($languages as $language) {
    ...
    if (($_SESSION["lang"] ?? null) && $language["language_code"] == ($_SESSION["lang"] ?? null)) {
      $l["active"] = 1;
...
if ($templateDir == false && $route[0] == "auth") { $templateDir = "login"; }
  }
  ...
  $blogcontent = is_array($blog) ? ($blog["content"] ?? '') : '';
  $blogtitle   = is_array($blog) ? ($blog["title"] ?? '')   : '';
  $blogimage   = is_array($blog) ? ($blog["image_file"] ?? '') : '';
  ...
  if (is_array($user) && $user["auth"]):
    ...
    for($i = 0; $i < count($menus); $i++){
        $menu_lang_name = json_decode($menus[$i]["menu_name_lang"] ?? '[]', 1)[$user["lang"] ?? ''] ?? null;
    ...
    for($i = 0; $i < count($menus); $i++){
        $menu_lang_name = json_decode($menus[$i]["menu_name_lang"] ?? '[]', 1)[$selectedLang] ?? null;
        if(!$menu_lang_name){
        $menus[$i]["menu_name"] = $menus[$i]["name"];
        } else {
            $menus[$i]["menu_name"] = $menu_lang_name;
  ...
  // Default undefined template variables
  ...
  $earnList      = $earnList      ?? null;
  $ordersList    = $ordersList    ?? null;
  $paginationArr = $paginationArr ?? null;
  $methodList    = $methodList    ?? null;
  $methoList     = $methoList     ?? null;
  $panel_logs    = $panel_logs    ?? null;
  $PaytmQR       = isset($PaytmQR) && is_array($PaytmQR) ? $PaytmQR : ["method_type" => null];
  $PaytmQRimage  = $PaytmQRimage  ?? null;
  $PhonepeQRimage= $PhonepeQRimage?? null;
  $WhatsApp      = isset($WhatsApp) && is_array($WhatsApp) ? $WhatsApp : ["method_type" => null];
  $whatsappnumber    = $whatsappnumber    ?? null;
  $whatsappposition  = $whatsappposition  ?? null;
  $whatsappstatus    = $whatsappstatus    ?? null;
  $whatsappvisibility= $whatsappvisibility?? null;
  $bankPayment   = isset($bankPayment) && is_array($bankPayment) ? $bankPayment : ["method_type" => null];
  ...
  $affiliatessystem = $affiliatessystem ?? null;
  $Promotion     = $Promotion     ?? null;
  $updates       = $updates       ?? null;
  $captcha       = $captcha       ?? null;
  $google_login_content = $google_login_content ?? '';
  $stylesheet    = isset($stylesheet) && is_array($stylesheet) ? $stylesheet : [];

  if (($_SESSION["msmbilisim_userlogin"] ?? 0) != 1 || ($user["client_type"] ?? 0) == 1) {
    echo $twig->render(
      $templateDir . '.twig',
      array(
        'site' => [
          ...
          "site_userr" => $settings["site_userr"] ?? '',
          "site_userrr" => $settings["site_userrr"] ?? '',
          "site_marqueen" => $settings["site_marqueen"] ?? '',
          "site_marqueens" => $settings["site_marqueens"] ?? '',
          "site_video" => $settings["site_video"] ?? '',
          ...
          "tgbottokenuser" => $settings["tgbottokenuser"] ?? '',
    ...
    $bronz_statu  = $bronz_statu  ?? 0;
    $silver_statu = $silver_statu ?? 0;
    $gold_statu   = $gold_statu   ?? 0;
    $bayi_statu   = $bayi_statu   ?? 0;
    $statusu      = $statusu      ?? 'VIP';
    ...
    echo $twig->render(
      ...
      [
        'site' => [
          ...
          "ns1" => (isset($childpanel_nameservers) && is_array($childpanel_nameservers)) ? ($childpanel_nameservers["ns1"] ?? '') : '',
          "ns2" => (isset($childpanel_nameservers) && is_array($childpanel_nameservers)) ? ($childpanel_nameservers["ns2"] ?? '') : '',
          "snow_colour" => $settings["snow_colour"] ?? '',
          "snow_effect" => $settings["snow_effect"] ?? '',
          "popup" => $settings["popup"] ?? '',
        ...
        'paymentsList' => $methodList ?? null,
        'integrationsList' => $methoList ?? null,
        'payments' => $paymentHistory ?? null,
        'chilpanel_logs' => $panel_logs ?? null,
        'PaytmQRimage' => $PaytmQRimage ?? null,
        "phonpe_qr_image" => $PhonepeQRimage ?? null,
        'PaytmQR' => isset($PaytmQR) && is_array($PaytmQR) ? ($PaytmQR["method_type"] ?? null) : null,
        'WhatsApp' => isset($WhatsApp) && is_array($WhatsApp) ? ($WhatsApp["method_type"] ?? null) : null,
        'whatsappnumber' => $whatsappnumber ?? null,
        'whatsappposition' => $whatsappposition ?? null,
        'whatsappstatus' => $whatsappstatus ?? null,
        'whatsappvisibility' => $whatsappvisibility ?? null,
        'bankPayment' => isset($bankPayment) && is_array($bankPayment) ? ($bankPayment["method_type"] ?? null) : null,
        'bankList' => $bankList ?? null,
        'status' => $route[1] ?? '',
        'earn' => $earnList ?? null,
        'orders' => $ordersList ?? null,
        'pagination' => $paginationArr ?? null,
        'contentText' => $content1,
        'contentText2' => $content2,
        'headerCode' => $settings["custom_header"],
        'footerCode' => $settings["custom_footer"],
        ...
        "massorders" => $_SESSION["massorders"] ?? null,
        "massorder_seen" => $_SESSION["massorder_seen"] ?? null,
        ...
        "child_panel_currencies" => $choose_currencies ?? null,
  ...
  if (route(0) != 'admin' && route(1) != 'v2' && route(0) != 'ajax_data') {
    ...
    if (($_SESSION['msmbilisim_userlogin'] ?? 0) != 1) {
